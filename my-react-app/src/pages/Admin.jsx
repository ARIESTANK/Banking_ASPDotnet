  ...
  return (
    <div className="anim-slide-up">
      ...
      <div className="table-wrapper">
        <table className="admin-table">
          <thead>
            <tr>{["User","Type","Amount","Description","Date","Status"].map(h => <th key={h}>{h}</th>)}</tr>
          </thead>
          <tbody>
...
export default function AdminDashboard() {
  ...
  const handleNewTx = async (e) => {
    ...
    // ── Validate form ──
    if (!txForm.type || !txForm.amount || !txForm.accountNumber) {
    ...
    const createTransactionDto = {
      senderId: parseInt(localStorage.getItem("authUser")),
  ...
  const deleteUser = async(id) => {
    const response = await fetch(`http://localhost:5288/api/User/${id}`,{method:"DELETE"})
    if(response.status==200){
    usersListFetch();
    showToast("User deleted successfully!", "success");
    }else{
      showToast("Failed User Delete!", "failed");
    }
  };
  ...
  const adminNameFetch = async(id)=>{
    ...
    if (response.status === 200) {
      ...
      setAdminName(null) ; // no account
    }
  }
  useEffect(()=>{
    adminNameFetch(localStorage.getItem("authUser"));
    usersListFetch();
    transactionAmount();
  },[])
          ...
          <nav className="nav-inner" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div className="desktop-nav" style={{ gap: 12 }}>
              ...
              onClick={()=>{
                ...
                style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(201,168,76,0.1)", color: "#c9a84c", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
                <LogOutIcon /> Logout
              </button>
            </div>
        ...
        <form onSubmit={handleNewTx}>
          <Field label="Sender (User)">
            ...
            style={{
             ...
             onChange={e => setTxForm(f => ({ ...f, userId: e.target.value }))}
            value={adminName} 
            disabled
            />
          </Field>
